...
> 💡 **후속 실측/실증 로드맵**: 물리 하드웨어(TPM 2.0), 라이브 EVM 테스트넷, 10,000 노드 파티션 및 수학적 정형 증명을 다루는 후속 계획은 **[로드맵 2 (ROADMAP2.ko.md)](ROADMAP2.ko.md)**를 참조하십시오.
...
1. **`contracts/AEREscrow.sol` (Solidity 0.8.24+)**
   ...
   - **플러머 원칙 즉시 정산 & 낙관적 가변 타임락**:
     - 정상 영수증 서명 즉시 락업/체류 0초로 **100% 즉각 출금 및 소비 보장**.
     - 의뢰인 침묵 시 카운트다운 가동 후 자동 방출.
   - **시간 유동성 프리미엄 & 감시탑 일시정지 (Risk-Return)**:
     - 오프라인 의뢰인이 타임락을 $72\text{h} \sim 168\text{h}$로 연장할 경우 작업자 대기 보상을 위한 시간 프리미엄($\Delta B_{\text{time}}$) 의무 가산.
     - 의뢰인 부재 시 스키마 결함 감지 감시탑(Watchtower)의 1가스 타이머 일시정지(Pause) 인터페이스.
   - **비처벌적 분쟁 장부 & 차기 수익 우선 상계 (Priority Netting)**:
     - 시스템의 임의 신용 처형 배제 (오직 자연 반감기만 강제).
     - 사후 분쟁 확정 시 음수 잔고($-B$) 및 전과 사실만 기표되며, 차기 과제 수익에서 최우선 자동 상계.
...
3. **`contracts/DisputeVerifier.sol` (Solidity 0.8.24+)**
   - **물리 공간 대화형 이분 탐색 및 다중 양식 검증기 (부록 C.1 실현)**:
     - 3D 옥트리 머클 리프 경로 및 단일 센서 프레임 ZK-SNARK 간이 증명(20만 가스 이하) 온체인 검증.
     - [시각+궤적+모터전류+배터리] 다중 물리 보존량 교차 검증을 통한 센서 기만 경제적 억제.

4. **`contracts/VendorCARegistry.sol` (Solidity 0.8.24+)**
   - **반도체 Root CA 탈중앙 레지스트리 및 오픈 실리콘 앵커 (부록 C.2 실현)**:
     - 소인수분해 취약점 수학적 증거($p \times q = N$) 투하 시 $O(1)$ 자율 무효화 및 RFC 5280 CRL 머클 릴레이.
     - 상용 벤더 외 RISC-V OpenTitan/Keystone 및 탈중앙 Web-of-Trust 앵커 병렬 수용으로 국가 제재 및 단일 벤더 블랙리스트 검열 무력화.
