...
* **`contracts/AEREscrow.sol` (Solidity 0.8.24+)**
  ...
  3. **결정론적 반증 검증 분쟁 (Deterministic Fraud Dispute)**:
...
| **`verifier.py`** | **WASM Fuel-metered 격리 샌드박스**: 배관공의 원칙에 따라 수혜자가 결과물을 로컬에서 돌려볼 때 호스트 머신이 해킹당하지 않도록, Wasmtime 기반 선형 메모리 격리 및 연산량(Fuel) 계측 환경 제공. |
| **`state.py`** | **신뢰의 상전이(Phase Transition) & 반감기 폭포**: 인위적 선형 벌점($-n$)을 철저히 배제하고, 배신 밀도가 $\Omega_c$를 넘는 순간 비트 시프트 반감기(`>> 1`)를 가속화하여 평판 질량을 $A_0$로 수직 낙하시키는 통계물리학적 상전이 엔진. |
...
| **`p2p_mesh.py`** | **P2P 가십 라우터 & 토폴로지 보이콧**: 분산 메시 네트워크 상에서 배신 증거(Fraud Claim)를 가십 전파하고, 배신 노드로 향하는 모든 라우팅 엣지를 자율적으로 단절. |

---

### Phase 4. 경제학 & 물리학 시뮬레이터 (`simulation/`)
AER의 경제학적 무결성과 물리학적 수렴성을 누구나 터미널에서 즉시 검증할 수 있는 시뮬레이션 스위트입니다.
