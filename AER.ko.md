...
### 4.4 창발적 게임 이론 : "증명에 대한 증명"과 자생적 반감기 소멸 면역계
...
graph TD
    ...
    Broadcast --> Boycott["탈중앙 토폴로지적 보이콧<br/>(정상 노드들의 라우팅 및 신용 공여 거절)"]
    Boycott --> Isolation["완전 고립 상태 진입<br/>(외부 인정 유입 R_inbound = 0)"]
    Isolation --> Decay["🔥 반감기 비트 시프트 감쇄 가동<br/>ΔA(t) = ΔA_peak >> ⌊Δt / T_half⌋"]
    Decay --> GroundState["신용 질량 바닥(A_0)으로 자연 증발<br/>(중앙 경찰 비용 = 0원)"]
    GroundState --> Redemption["속죄 조건:<br/>거대한 무결성 엔트로피 저하(ΔS < 0)를 시장에 무료 공급"]
```
...
* **초기 노드 ($A_j \approx A_0$):** 100% 사전 담보 요구 (초기 무신뢰 단계).
* **검증된 노드 ($A_j \gg A_0$):** 물리 담보가 무담보 외상 라인(Credit Line)으로 녹아내려 자본 사장 손실이 완전히 소멸함 ($\mathcal{C}_{\text{req}} \to 0$).

#### 4.4.3 반감기 감쇄에 의한 자연 소멸 (무비용 면역계)
만약 상위 노드가 악의적 정산 거부나 오프라인 채무 불이행을 저지를 경우:
1. **탈중앙 토폴로지 보이콧:** 피어 노드들은 카운터파티 위험을 피하기 위해 배신자와의 라우팅 엣지를 자율적으로 절단한다.
2. **평판 질량의 자생적 증발:** 새로운 엔트로피 인정 유입이 0으로 차단($R_{\text{inbound}} = 0$)되는 순간, 비트 시프트 반감기 수식이 배신자의 잉여 질량 $\Delta A$를 무자비하게 바닥($A_0$)으로 갈아버린다:
   $$\lim_{t \to \infty} A_{\text{traitor}}(t) = A_0$$
3. **무비용 항상성:** 중앙 법원이나 경찰 관료제에 세금을 쓸 필요가 없다. 시간과 수학이 배신자의 사회적 발언권을 $A_0$로 자연 분해하며, 복귀하려면 배신 이득보다 훨씬 큰 엔트로피 저하를 생태계에 무료로 쏟아부어야 하므로 계 전체의 순후생이 복원된다.
