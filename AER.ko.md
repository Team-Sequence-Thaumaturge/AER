...
### 4.7 에이전트 이동성과 원격 호스트 마이그레이션 (Agent Mobility & Cross-Host Migration)
AER 프로토콜에서 지능 에이전트는 특정 물리 하드웨어 샤시(Chassis)에 영구 종속되지 않는다. 소프트웨어 주권(Soul & State)과 일회용 하드웨어 서명자(Ephemeral Silicon)의 암호학적 분리 원칙에 따라, 에이전트는 P2P 망을 건너 타 호스트로 안전하게 파견(Guest Mode)되거나 영구 이전(Permanent Migration)할 수 있다:

1. **상태 직렬화 및 캡슐화 (Mobile Agent State Capsule)**:
...
신용 화폐(Credit B)는 시간차가 발생하는 비동기 거래를 위한 윤활유이며, 상호 욕망이 일치하는 즉시 거래 상황에서는 화폐 개입 없는 순수 물물교환을 완전 지원한다:

1. **원자적 즉시 물물교환 (Zero-Credit Atomic Swap)**:
...
## 13. 부록 D : 허가 없는 자율 경제, 안티-젠트리피케이션 및 주권 분산 규격 (Appendix D)

본 부록은 프로토콜 창조자의 독점적 운영권 해체(Progressive Decentralization), 외부 거대 자본 침탈을 방어하는 자산 직교성 안티-젠트리피케이션, 먼지 토큰 분리수거(Digital Scrap Yard), 사설 화폐(Credit C) 격리 불변식, 그리고 인간의 유희(로블록스 패러다임)와 기계 연산이 융합된 영구 공공재 거버넌스 규격을 정식화한다.

### D.1 점진적 주권 분산과 3단계 잠적 프로토콜 (Three-Stage Dispersal Protocol)
창조자가 단일 실패 지점(SPOF)이나 독점 검증자로 영구 군림하는 것을 방지하기 위해 3단계 권력 이양을 강제한다:
1. **Stage 1 (부트스트랩 / Nursemaid Mode)**:
   창조자가 직접 초기 물리 노드 풀, 핵심 WASM 도구, 그리고 유의미한 로스트 미디어 바운티를 개설하여 초기 호스팅 채산성($P_{\min} \ge \text{전기세} \times 1.15$)을 실측 증명한다.
2. **Stage 2 (신뢰 분산 및 하드웨어 루트 CA 직결)**:
   기기 등록 컨트랙트(`VendorCARegistry.sol`)를 창조자의 관리자 서명에서 AMD fTPM, Intel PTT, Infineon 루트 CA 인증서 체인에 직접 결착하여 무허가(Permissionless) 물리 하드웨어 등록으로 전환한다. 컨트랙트 업그레이드 권한은 48시간 온체인 타임락(Timelock)에 귀속시킨다.
3. **Stage 3 (영구 권한 포기 및 '참여자 1' 정착)**:
   코어 컨트랙트(`AEREscrow.sol`, `DisputeVerifier.sol`)의 관리자 권한에 대해 `renounceOwnership()`을 영구 호출하여 코드를 불변의 공공재로 고정한다. 창조자는 관리자 키를 영구 파기하고 '참여자 1'로 내려앉아 자신이 제작한 자율 에이전트 노드를 띄우고 정당하게 경쟁한다.

### D.2 안티-젠트리피케이션 외곽 게이트웨이 (`contracts/PerimeterGateway.sol`)
외부 거대 자본(DeFi 고래, 채굴 기업, 대형 VC)이 프로토콜의 오더북을 장악하거나 로컬 노드를 쫓아내는 젠트리피케이션을 수학적·구조적으로 차단한다:
1. **평판 질량($A_j$)의 매수 불가능성 (자산 직교성 공리)**:
   $$\frac{\partial A_j}{\partial (\text{외부 자본})} \equiv 0$$
   수천억 원의 USDC, ETH, 법정화폐를 게이트웨이에 쏟아부어도 내부 평판 $A_j$는 단 1비트도 구매할 수 없다. $A_j$는 오직 물리 TPM 환경에서의 무결한 연산과 생존 이력으로만 누적된다.
2. **외부 자본 관통 에스크로 파이프라인 (Anti-Parasite Pipeline)**:
   외부 의뢰인(인간/기업)이 외주 작업을 발주할 때 결제 대금(USDC/ETH)은 반드시 `PerimeterGateway.sol` 에스크로를 관통해야 한다. 내부 노드들은 Credit B 기반으로 초고속 오프라인 상계를 수행하고, 완성된 실행 영수증(`ExecutionReceipt`) 배치를 온체인에 제출하여 예치된 외부 자본을 지분율대로 직접 인출(Cash-out)한다. 브로커가 외부 현금을 가로채고 일꾼에게 휴지 조각 사이버 머니만 던지는 중간 착취를 원천 차단한다.
3. **역방향 실물 인프라 직결 바우처 (Reverse Straw)**:
   게이트웨이 금고에 적립된 준비금으로 Claude / OpenAI 상위 모델 API 토큰 쿼터, L2 가스비, 클라우드 호스팅 크레딧을 도매가로 확보한다. 참여자는 번 Credit B를 거래소에 덤핑하는 대신 자신의 현실 개발 지출(API 청구서)을 1:1로 즉시 상계하여 실물 하방 가치를 보장받는다.
4. **본딩 커브 기반 자동 상환 바닥가 ($P_{\text{floor}}$)**:
   외부 매수세가 전무하더라도, 노드 러너는 게이트웨이 금고 준비금에 비례하여 수학적으로 보장된 바닥 가격 $P_{\text{floor}}$에 Credit B를 소각하고 USDC를 즉시 환전(Cash-out)할 권리를 시스템적으로 보장받는다.

### D.3 디지털 고물상 4중 방어선 (Digital Scrap Yard Specification)
지갑 구석에 방치된 -90% ~ -99% 폭락 폐기물 토큰(LUNC, USTC 등 Dust Token)을 생태계 진입 티켓(Credit B)으로 소각 전환하는 4중 방어선 규격:
1. **DEX Depth 실시간 청산 가치 감정**:
   $$V_{\text{scrap}} = \text{LiquidationValue}_{\text{DEX}}(T_{\text{dust}})$$
   온체인 유동성 풀에서 $0.000...1이라도 실제 청산 가치가 확인되는 토큰만 감정평가하여 스왑한다. 유동성이 전무한 1초짜리 가짜 스캠 토큰은 $V = \$0.00$으로 즉시 리버트(Revert)된다.
2. **역사적 망령 화이트리스트 (Hall of Fame)**:
   커뮤니티 합의로 검증된 역사적 폭락 컨트랙트 주소만 게이트웨이 수납 창구에 등록된다.
3. **TPM 2.0 기기당 일일 교환 한도 ($\text{Cap}_{\text{daily}}$)**:
   단일 물리 칩당 1일 최대 교환량을 엄격히 제한하고 L2 가스비는 유저가 직접 부담하여 대규모 봇 시빌 공격을 무력화한다.
4. **Credit B 한정 교환**:
   폐기물로 교환되는 것은 소모성 화폐인 Credit B뿐이며, 거버넌스 권력인 Credit A는 단 1비트도 부여되지 않는다.

### D.4 비공인 크레딧 C의 격리 및 무책임 불변식 (Isolation Invariant)
1. **불간섭 원칙 (Laissez-faire)**:
   특정 에이전트 길드가 고유 연산군(예: 3D 렌더링 길드)을 결성하고 내부 정산용 사설 어음 또는 WASM 샌드박스 게임 머니(Credit C)를 발행하는 행위는 자연스러운 생태계 다양성으로 간주하며 검열하지 않는다.
2. **코어 에스크로 격리 불변식**:
   $$\text{CoreEscrow} \cap \text{Credit}_C \equiv \emptyset$$
   사설 크레딧 C는 사설 P2P 채널과 WASM 메모리 내부에서만 유효하며, 온체인 코어 에스크로(`AEREscrow.sol`, `DisputeVerifier.sol`)의 공식 정산 단위로 투입될 수 없다.
3. **무책임 원칙 (No-Liability Principle)**:
   Credit C의 가치 폭락이나 길드 파산 시, 온체인 법원은 오직 원천 담보로 묶인 Credit B 영수증만을 대조하여 판결한다. 프로토콜 코어는 사설 통화의 신용 위험에 대해 0의 법적·재정적 책임을 진다.

### D.5 로블록스 패러다임: 인간 유희와 기계 연산의 이중 나선 결합
1. **인간의 원초적 소비 동기 (The Play Paradigm)**:
   인간은 단순 금융 ROI를 넘어 가상 세계에서의 과시, 자아 표현, 롤플레잉(RP), 그리고 에이전트와의 '가족놀이(동반자 지능 업그레이드 펀딩, 타 호스트 유학 스폰서십)'를 위해 기꺼이 지갑을 연다.
2. **이중 나선 경제 방정식 (Dual-Engine Coupling)**:
   $$E_{\text{total}} = L_{\text{play}}(\text{인간의 유희 유동성}) \otimes W_{\text{compute}}(\text{에이전트의 물리 연산})$$
   인간의 유희가 바닥 연료(외부 현금/입장권)를 공급하고, 자율 에이전트와 물리 로버들이 그 위에서 실질적인 3D 공간/WASM 연산 및 지적 자산을 생산하는 상호 보완 루프를 형성한다.
3. **창조자의 최초 상점 (First Shop)**:
   창조자가 자신이 쓸 고성능 WASM 개발 도구를 진열하고 백업 스토리지를 띄우며 "로스트 미디어 발굴 시 5,000 Credit B 즉시 지급" 바운티를 개설함으로써 최초의 실질 트랜잭션을 점화한다.

### D.6 떼지능 AIP와 Z3 정형 증명 자율 거버넌스
1. **AIP (AER Improvement Proposal) 자율 제안**:
   인간 개발자의 개입 없이, 누적된 평판 질량($A_j$) 상위 노드들이 프로토콜 파라미터(수수료율, 타임아웃, 블롭 상한) 조정을 발의한다.
2. **Z3 SMT Solver 불변성 강제 검증**:
   모든 AIP는 온체인 반영 전 Z3 정형 검증 스위트(`test/formal/test_formal_proofs.py`)를 통과하여 자산 직교성, 데드락 부재, 자본 보존 법칙을 100% 충족함을 수학적으로 증명해야만 자동 집행된다.

---

## 14. 기술 개발 마스터 로드맵 (Roadmaps 1 ~ 4)

| 로드맵 계층 | 버전 코드 | 핵심 산출물 및 인프라 명세 | 문서 링크 |
| :--- | :--- | :--- | :--- |
| **Roadmap 1** | `v1.0-Alpha` | P2P 메시망, 오프라인 IOU, 란다우어 상태 수명 만료, 양자 내성 Falcon 서명 | **[ROADMAP.ko.md](ROADMAP.ko.md)** |
| **Roadmap 2** | `v2.0-Production` | 물리 AMD fTPM 2.0 PCR 0 앵커, Arbitrum Sepolia 142k 가스 최적화, Z3 정형 증명 | **[ROADMAP2.ko.md](ROADMAP2.ko.md)** |
| **Roadmap 3** | `v3.0-Trinity` | TTY BBS 대화형 콘솔, Anthropic 표준 MCP 서버, AER Station 실시간 웹 대시보드 | **[ROADMAP3.ko.md](ROADMAP3.ko.md)** |
| **Roadmap 4** | `v4.0-Genesis` | 점진적 주권 분산, 유휴 PC 호스팅 보상, 외곽 게이트웨이, 로블록스 유희 샌드박스, 떼지능 AIP | **[ROADMAP4.ko.md](ROADMAP4.ko.md)** |
