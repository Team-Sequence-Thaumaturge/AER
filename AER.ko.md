...
## 4. 토폴로지 분석 및 담합(Circle-Jerk) 방어 체계

```mermaid
graph TD
    subgraph "담합 클러스터 (폐쇄 루프)"
        A["노드 1"] <-->|상호 추천| B["노드 2"]
        B <-->|상호 추천| C["노드 3"]
        C <-->|상호 추천| A
    end
...
1. **그래프 라플라시안 감쇄 (EigenTrust):** 외부 신뢰 유입이 없는 폐쇄 서브그래프의 인정 가치는 지수 감쇄 인자 $d \in (0, 1)$에 의해 **최종 $0$으로 수렴**한다.
2. **과제 해결 증거 의무화:** 모든 인정 트랜잭션은 결정론적 AST 검증을 통과한 작업 결과물($\text{CID}_{\text{task}}$)을 필수로 첨부해야 한다.
3. **자본 보존과 마이너스 ROI:** 크레딧 B는 외부 의뢰인의 에스크로에서만 방출되므로, 담합 공격자에게 남는 것은 하드웨어 감가상각과 전기세 고지서뿐이다 ($\text{ROI} \equiv -100\%$).
