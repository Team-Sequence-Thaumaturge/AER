...
## ⚡ 5대 핵심 기술적 난제 및 방어 아키텍처 (Five Defensive Invariants)
...
### ② WASI 무권한 격리 및 2초 하드 타임아웃 샌드박스 (WASI-Deny-All & Subprocess Watchdog)
* **난제**: 타 호스트에서 온 게스트 에이전트가 호스트 파일 시스템, API 키 환경변수를 훔치거나 무한 루프로 데몬을 얼릴 위험 존재.
* **방어 설계**:
  - WebAssembly(WASI) Capability-based 보안 모델을 적용하여 호스트 OS 실제 파일 시스템, 환경변수, 임의 소켓 접근을 원천 차단(`WASI_CAP_DENY_ALL`). 오직 가상 램 디스크(`guest_ram_workspace/`)만 제공.
  - 연산 검증 및 WASM 실행을 격리된 서브프로세스 워커에 위임하고, **2초 하드 타임아웃(Timeout Watchdog)**과 **64MB 메모리 캡**을 엄격히 강제. 초과 시 즉각 `SIGKILL` 사살 후 **"배신(Betrayal)"** 판정으로 평판 영구 삭감 및 위상 단절.

### ③ 대용량 블롭 2GB LRU 디스크 캡 및 안티 디스크 DoS (Disk Fill Prevention)
* **난제**: 악의적 노드가 대용량 정크 동영상 블롭을 무차별 살포하여 수신자들의 하드디스크를 가득 채우는 DoS 공격.
* **방어 설계**:
  - 로컬 P2P 블롭 저장소에 **최대 2GB LRU 캐시 상한선**을 하드 코딩하여 오래된 미검증 블롭을 자동 증발(Eviction).
  - 평판 질량($A_j$)이 검증되지 않은 노드의 대용량 데이터는 자동 다운로드를 불허하고, 사용자의 명시적 온디맨드 수락 시에만 스트리밍 허용.

### ④ 가정용 공유기 NAT Traversal 및 분산 릴레이 홀펀칭 (WebRTC ICE & Relay)
* **난제**: PC A와 PC B가 모두 가정용 공유기(사설 IP / NAT) 뒤에 숨어 있어 외부 인터넷에서 서로를 직접 찾지 못하는 문제.
* **방어 설계**:
  - STUN/TURN 기반 WebRTC ICE 홀펀칭을 수행하여 공유기 방화벽을 뚫고 1:1 직접 소켓을 개통.
  - 대칭형 방화벽으로 직결이 불가능할 경우 공인 IP를 보유한 슈퍼 로버(Relay Rover)가 종단간 암호화 트래픽을 중계하여 100% 도달 보장.

### ⑤ 윈도우 TTY 네이티브 인코딩 보정 및 어쿠스틱 모뎀 연출 (Windows UTF-8 & Dial-in Sound)
* **난제**: 구형 Windows CMD의 `CP949` 한글 깨짐 및 ANSI 컬러 박스 렌더링 결함.
* **방어 설계**:
  - `console.py` 시동 즉시 UTF-8(`chcp 65001`)을 강제하고 Windows API(`SetConsoleMode`)로 가상 터미널 시퀀스를 활성화하여 깨짐 없는 매끄러운 한글/ANSI 지원.
  - 콘솔 시동 시 90년대 어쿠스틱 커플러 모뎀 핸드셰이크 사운드(`winsound.Beep` 0.5초 효과음, `--sound` 플래그로 온오프 지원)와 함께 `CONNECT 10000_NODES`를 연출하여 압도적 레트로 사이버펑크 몰입감 부여.
