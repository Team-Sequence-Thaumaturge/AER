...
* **Hermetic WASI Sandbox & Anti-Disk DoS Shield**: Visiting external agents run under zero-privilege capability isolation (`WASI_CAP_DENY_ALL`) without accessing host files or environmental API keys, while a 2GB LRU storage cap protects disks from malicious broadcast flooding.
* **Autonomous NAT Traversal & Distributed Relay**: Nodes trapped behind domestic home routers (private IPs/symmetric firewalls) connect directly via STUN/TURN WebRTC ICE hole punching, with Super Rover encrypted relays preserving 100% mesh reachability worldwide.
